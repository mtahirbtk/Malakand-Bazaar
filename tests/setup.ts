...
// needs a stand-in, not real blob semantics. (lib.dom's types declare both as
// always present, so an `if (!URL.createObjectURL)` guard here would narrow
// to `never` — assign unconditionally instead, falling back only at runtime.)
URL.createObjectURL = URL.createObjectURL ?? vi.fn(() => "blob:mock-object-url");
