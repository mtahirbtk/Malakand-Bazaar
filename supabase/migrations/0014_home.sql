-- 0014 — the homepage (§2.3 #25, §6.3).
--
-- One RPC for the whole page: readiness flags, category shelves (only the
-- ones that clear `home.min_listings_per_shelf`), trending (gated on
-- `home.min_trending_listings`), top sellers (gated on
-- `home.min_rated_sellers`), and a couple of honest counters for the
-- "marketplace is opening" state. Nothing here fabricates a shelf below
-- threshold — the front end renders exactly the sections this returns.
...
as $$
  ...
  top_sellers as (
    select coalesce(jsonb_agg(
      jsonb_build_object(
        'id', s.id, 'slug', s.slug, 'name', s.name, 'localityLabel', s.locality_label,
...
comment on function public.fn_home_payload is
  'GET /api/home: readiness flags + category shelves + trending + top sellers + counters, one round trip.';
